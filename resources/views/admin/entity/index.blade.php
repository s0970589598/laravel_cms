...
<script type="text/html" id="action">
    ...
    <a href="javascript:;" class="layui-table-link" title="添加默认選單，用于精细化权限控制" style="margin-left: 10px" onclick="addEntityMenus('<% d.menuUrl %>')"><i class="layui-icon layui-icon-menu-fill"></i></a>
...
@section('js')
    <script>
        ...
        function deleteEntity (url) {
            layer.confirm('删除模型将删除模型相关的所有数据（模型、模型字段、模型分類、模型内容等），请谨慎操作！确定要删除？', function(index){
                layer.prompt({
                    formType: 1,
                    title: '请输入登录密码',
        ...
        function copyEntity (url) {
            layer.confirm('复制模型将新建一个和当前模型一样的模型（資料庫表结构、表单定义等信息一致），确定要复制？', function(index){
                layer.prompt({
                    formType: 0,
                    title: '请输入新模型的資料庫表名称',
                }, function(value, index, elem){
        ...
        function addEntityMenus (url) {
            layer.confirm('添加模型的默认選單是為了对指定模型进行单独的权限控制，添加選單时如遇到同名的選單将略过不处理，确定要添加？', function(index){
                $.ajax({
                    url: url,
                    success: function (result) {
                        if (result.code !== 0) {

...
## 项目简介
`lightCMS`是一個轻量级的`CMS`系统，也可以作為一個通用的后台管理框架使用。`lightCMS`集成了用户管理、權限管理、Log管理、選單管理等后台管理框架的通用功能，同时也提供模型管理、分類管理等`CMS`系统中常用的功能。`lightCMS`的**代码一键生成**功能可以快速對特定模型生成增删改查代码，极大提高開發效率。

`lightCMS`基于`Laravel 6.x`開發，前端框架基于`layui`。
...
* 標簽管理
* 配置管理
* 模型、模型字段、模型内容管理（后台可自定義业务模型，方便垂直行业快速開發）
* 會員管理
* 評論管理
* 基于Tire算法的敏感詞过滤系统
* 普通模型增删改查代码一键生成
...
```bash
# 此處權限设置為777只是為了演示操作方便，实际只需要给Web服務器写入權限即可
sudo chmod 777 -R storage/ bootstrap/cache/
```
新建一份环境配置，并配置好資料庫等相關配置:
```base
cp .env.example .env
```
初始化系统：
...
server {
    ...
    location ~ \.php$ {
        ...
        #不同配置對应不同的环境配置文件。比如此處应用会加载.env.pro文件，默認不配置会加载.env文件。此處可根据项目需要自行配制。
...
## 權限管理
基于角色的權限管理。只需新建好角色，给對应的角色分配好相应的權限，最后给用户指定角色即可。`lightCMS`中權限的概念其实就是選單，一条選單對应一個`laravel`的路由，也就是一個具体的操作。

### 選單自動获取
只需要按约定方式写好指定路由的控制器注譯，则可在[選單管理](/admin/menus)頁面自動添加/更新對应的選單信息。例如：
```php
/**
 * 角色管理-角色列表
 *
...
需要注意的是，程序可以自動获取選單，但是選單的层级關系还是需要在后台手动配置的。
...
`lightCMS`采用中间表（content_tags）来实现標簽和模型内容的多對多關联關系。

## 模型管理
`lightCMS`支持在后台直接創建模型，并可對模型的表字段进行自定義设置。设置完模型字段后，就不需要做其它工作了，模型的增删改查功能系统已经内置。

> 小提示：如果需要對單独的模型进行權限控制，可以在模型管理頁面点擊`添加默認選單`，系统会自動建立好相应模型的相關選單项。

這里說明下模型的表單验证及編輯的保存和更新處理。如果有自定義表單验证需求，只需在`app/Http/Request/Admin/Entity`目入下創建模型的表單请求验证類即可。類名的命名规则：**模型名+Request**。例如`User`模型對应的表單请求验证類為`UserRequest`。

如果想自定義模型的新增/編輯前端模板，只需在`app/resources/views/admin/content`目入下創建模板文件即可。模板文件的命名需遵循如下命名规则：**模型名_add.blade.php**。例如`User`模型對应的模板文件名為`user_add.blade.php`。

如果想自定義模型的保存和更新處理逻辑，只需在`app/Http/Controllers/Admin/Entity`目入下創建模型的控制器類即可，`save`和`update`方法实现可参考`app/Http/Controllers/Admin/ContentController`。類名的命名规则：**模型名+Controller**。例如`User`模型對应的控制器類為`UserController`。同理，如果想自定義列表頁，按上述规则定義`index`和`list`方法即可。

另外，模型内容在新增、更新、删除时系统会触發相应的事件，你可以监听這些事件做相应的业务處理。下表所示為相应的事件說明：
...
App\Events\ContentDeleted    |   Illuminate\Support\Collection $contents, App\Model\Admin\Entity $entity |  删除内容后  | $contents 為被删除内容的 App\Model\Admin\Content 對象合集

### 模型字段表單類型相關說明
對于支持远程搜索的`select`表單類型，編輯 API 搜索接口需返回的數據格式如下所示。code為0时, 表示正常, 反之異常。
```json
{
    "code": 0,
    "msg": "success",
...
對于短文本（input，自動完成）表單類型，編輯 API 接口需返回的數據格式如下所示：
```json
{
    "suggestions": [
        "cms",
...
對于`select`多選類型表單，默認資料庫保存值為半角逗号分隔的多個選擇值。當你设置字段類型為無符号整型时，資料庫会保存多個選擇值的求和值（當然前提是選擇值都是整型數據）。

### 搜索字段（$searchField）配置說明
通过配置搜索字段，可以很方便的在模型的列表頁展示搜索项。如下是一個示例配置：
```php
    public static $searchField = [
        'name' => '用户名', // input搜索類型。key 為字段名稱，value 為標題
        'status' => [ // key 為字段名稱，value 為相關配置
            'showType' => 'select', // 下拉框選擇搜索類型
            'searchType' => '=', // 說明字段在資料庫的搜索匹配方式，默認為like查询
            'title' => '狀態', // 標題
            'enums' => [ // select下拉搜索项
                0 => '禁用',
                1 => '启用',
        ...
        'recommend' => [ // key 為字段名稱，value 為相關配置
            'showType' => 'select', // 下拉框選擇搜索類型
            'searchType' => 'whereRaw', // 對于一些特殊的查询条件，無法通过上述普通的搜索匹配值来实现时，可将此值设置為 whereRaw
...
### 列表字段（$listField）配置說明
通过配置列表字段，可以很方便的在模型的列表頁展示列表项。如下是一個示例配置：
```php
...
通过配置列表操作项，可以很方便的在模型的列表頁操作列添加自定義連接。如下是一個示例配置：
```php
    public static $actionField = [
        // chapterUrl 是字段名（不一定是模型資料庫表的字段名，只要列表數據接口返回數據包含该字段即可）
        'chapterUrl' => ['title' => '章节', 'description' => '當前小說的所有章节'],
    ];
```

### 排序字段（$sortFields）配置說明
通过配置排序字段，可以很方便的在模型的列表頁自定義數據的排序规则。如下是一個示例配置：
```php
    public static $actionField = [
...
> 小提示：如果你是自定義模型，建议自定義模型继承`App\Model\Admin\Model`模型，方便對上述配置项进行自定義。

## 系统Log
`lightCMS`集成了一套简單的Log系统，默認情况下记入后台的所有操作相關信息，具体实现可以参考`Log`中间件。
...
可以通过配置`log_async_write`项来决定是否啟用異步写入Log（默認未启用），異步写入Log需要运行`Laravel`的[队列處理器](https://laravel.com/docs/5.8/queues#running-the-queue-worker)：
```bash
php artisan queue:work
```

## 代码一键生成
對于一個普通的模型，管理後台通常有增删改查相關的业务需求。如果系统模型管理自带的增删改查功能無法满足你的個性化需求，你可以使用一键生成代码功能。`lightCMS`拥有一键生成相關代码的能力，在建好模型的資料庫表结構后，可以使用如下`artisan`命令生成相關代码：
```bash
# config 為模型名稱 配置 為模型中文名稱
php artisan light:basic config 配置
```
成功執行完成后，会創建如下文件（注意：相關目入需要有写入權限）：

* routes/auto/config.php
路由：包含模型增删改查相關路由，应用会自動加载`routes/auto/`目入下的路由。
* app/Model/Admin/Config.php
模型：[$searchField](#搜索字段searchField配置說明) 属性用来配置搜索字段，[$listField](#列表字段listfield配置說明) 用来配置列表视圖中需要展示哪些字段數據。
* app/Repository/Admin/ConfigRepository.php
模型服务层：默認有一個`list`方法，该方法用来返回列表數據。需要注意的是如果列表中的數據不能和資料庫字段數據直接對应，则可對資料庫字段數據做相应转换，可参考`list`方法中的`transform`部分。
* app/Http/Controllers/Admin/ConfigController.php
控制器：默認有一個`$formNames`属性，用来配置新增/編輯表單请求字段的白名單。此属性必需配置，否则获取不到表單數據。参考 [request 對象的 only 方法](https://laravel.com/docs/5.5/requests#retrieving-input)
* app/Http/Requests/Admin/ConfigRequest.php
表單请求類：可在此類中编写表單验证规则，参考 [Form Request Validation](https://laravel.com/docs/5.5/validation#form-request-validation)
* resources/views/admin/config/index.blade.php
列表视圖：列表數據、搜索表單。
* resources/views/admin/config/index.blade.php
新增/編輯视圖：只列出了基本架構，需要自定義相關字段的表單展示。参考 [layui form](https://www.layui.com/doc/element/form.html)

最后，如果想讓生成的路由展示在選單中，只需在[選單管理](/admin/menus)頁面点擊**自動更新選單**即可。

## 敏感詞检测
如果需要對發表的内容（文章、評論等）进行内容审查，则可直接调用`LightCMS`提供的`checkSensitiveWords`函數即可。示例如下：
```php
...
## 圖片上傳
LightCMS中圖片默認上傳到本地服务器。如果有自定義需求，比如上傳到三方云服务器，可参考`config/light.php`配置文件中的`image_upload`配置项說明，自定義處理類需要实现`App\Contracts\ImageUpload`接口，方法的返回值數據结構和系统原方法保持一致即可。
```json
{
    "code": 200,
    "state": "SUCCESS",
...
此數據结構基本包含了分類的所有结構化信息。相關字段的含義也比较清楚，此處只對`path`字段做下說明：该字段是指當前分類的所有上级分類連，這樣可以很方便的知道某個分類的所有父级分類。比如圖中的`test`分類的path字段值為`[1, 2]`，那么很容易的知道它的父级分類是：游戲 射擊

## 前台相關
### 用户註冊登入
`LightCMS`集成了一套简單的用户註冊登入系统，支持微信、QQ、微博三方登入。三方登入相關配置请参考`config/light.php`。
...
## 說明
有問题可以提 issue ，為项目贡献代码可以提 pull request

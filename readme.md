...
## 项目简介
`lightCMS`是一个轻量级的`CMS`系统，也可以作為一个通用的后台管理框架使用。`lightCMS`集成了用户管理、权限管理、日志管理、菜单管理等后台管理框架的通用功能，同时也提供模型管理、分类管理等`CMS`系统中常用的功能。`lightCMS`的**代码一键生成**功能可以快速对特定模型生成增删改查代码，极大提高开发效率。

`lightCMS`基于`Laravel 6.x`开发，前端框架基于`layui`。

演示站点：[LightCMS Demo](http://lightcms.bituier.com/admin/login)。登录信息：admin/admin。请勿存储/删除重要数据，数据库会定时重設。
...
```bash
# 此处权限设置為777只是為了演示操作方便，实际只需要给web服务器写入权限即可
 ...
 * 角色管理-角色列表
 *
 * 取方法的第一行注释作為菜单的名称、分组名。注释格式：分组名称-菜单名称。
 * 未写分组名称，则直接作為菜单名称，分组名為空。
 * 未写注释则选用uri作為菜单名，分组名為空。
 */
...
## 标签管理
模型内容**打标签**是站点的一项常用功能，`lightCMS`内置了打标签功能。添加模型字段时选择表单类型為`标签输入框`即可。
...
如果想自定义模型的保存和更新处理逻辑，只需在`app/Http/Controllers/Admin/Entity`目录下创建模型的控制器类即可，`save`和`update`方法实现可参考`app/Http/Controllers/Admin/ContentController`。类名的命名规则：**模型名+Controller**。例如`User`模型对应的控制器类為`UserController`。同理，如果想自定义列表页，按上述规则定义`index`和`list`方法即可。
...
### 模型字段表单类型相关说明
对于支持远程搜索的`select`表单类型，后端 API 搜索接口需返回的数据格式如下所示。code為0时, 表示正常, 反之异常。
```json
{
    "code": 0,
    "msg": "success",
...
对于`select`多选类型表单，默认数据库保存值為半角逗号分隔的多个选择值。当你设置字段类型為无符号整型时，数据库会保存多个选择值的求和值（当然前提是选择值都是整型数据）。

### 搜索字段（$searchField）配置说明
通过配置搜索字段，可以很方便的在模型的列表页展示搜索项。如下是一个示例配置：
```php
    public static $searchField = [
        'name' => '用户名', // input搜索类型。key 為字段名称，value 為标题
        'status' => [ // key 為字段名称，value 為相关配置
            'showType' => 'select', // 下拉框选择搜索类型
            'searchType' => '=', // 说明字段在数据库的搜索匹配方式，默认為like查询
            'title' => '状态', // 标题
            'enums' => [ // select下拉搜索项
                0 => '禁用',
                1 => '启用',
...
可以通过配置`log_async_write`项来决定是否啟用异步写入日志（默认未启用），异步写入日志需要运行`Laravel`的[队列处理器](https://laravel.com/docs/5.8/queues#running-the-queue-worker)：
```bash
php artisan queue:work
```

## 代码一键生成
对于一个普通的模型，管理後台通常有增删改查相关的业务需求。如果系统模型管理自带的增删改查功能无法满足你的个性化需求，你可以使用一键生成代码功能。`lightCMS`拥有一键生成相关代码的能力，在建好模型的数据库表结构后，可以使用如下`artisan`命令生成相关代码：
```bash
...
此数据结构基本包含了分类的所有结构化信息。相关字段的含义也比较清楚，此处只对`path`字段做下说明：该字段是指当前分类的所有上级分类链，这样可以很方便的知道某个分类的所有父级分类。比如图中的`test`分类的path字段值為`[1, 2]`，那么很容易的知道它的父级分类是：游戏 射击
